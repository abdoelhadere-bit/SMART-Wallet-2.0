...
$amounts = $incomeModel->getAll($user_id);
$cats    = $incomeModel->getIncomeCategories();
    ...
    <div class="card overflow-x-auto">
        <table class="w-full">
            ...
            <?php foreach($amounts as $a): ?>
                <tr>
                    <td><?= (int)$a['id'] ?></td>
                    <td class="text-green-400 font-semibold"><?= number_format((float)$a['montant'], 2) ?> DH</td>
                    ...
                    <td class="text-center flex justify-center gap-2">
                        <button class="btn-warning editBtn"
                            data-id="<?= (int)$a['id'] ?>"
                            data-montant="<?= htmlspecialchars((string)$a['montant']) ?>"
                            data-category-id="<?= (int)$a['category_id'] ?>"
                            data-date="<?= htmlspecialchars((string)$a['date']) ?>"
                            data-desc="<?= htmlspecialchars((string)($a['description'] ?? '')) ?>">
                      Modifier
                    </button>

                        <a href="delete_income.php?id=<?= (int)$a['id'] ?>" class="btn-danger">
                            Supprimer
                        </a>
                    </td>
                </tr>
...
<div id="formModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center">
    <div class="bg-gray-800 w-96 p-6 rounded-2xl shadow-2xl border border-gray-700">
        ...
        <form id="incomeForm" class="space-y-4" action="add_income.php?file=incomes" method="POST" data-parsley-validate>
            ...
            <div>
                ...
                <textarea id="description" name="description" required
                    class="w-full p-2.5 border border-gray-600 rounded-xl bg-gray-900 text-gray-100 focus:ring-2 focus:ring-blue-500"
                    data-parsley-pattern="^[a-zA-Z0-9 À-ÿ'.,-]+$"
...
<script>
    ...
    btnAdd.onclick = () => {
        ...
        form.action = "add_income.php?file=incomes";
        form.reset();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.editBtn').forEach(btn => {
            ...
            form.action = "update_income.php";

            document.getElementById("incomeId").value = this.dataset.id;
            document.getElementById("amount").value = this.dataset.montant;
            document.getElementById("incomeDate").value = this.dataset.date;
